...
# Generated by m-notes CLI — do not edit manually.
# Logs session end to m-notes ONLY when MNOTES_SESSION_ID is explicitly set
# by the parent agent. Without this gate, every Claude Code session stop
# would create a brand-new empty session-log note (#931).
...
# Skip silently when the agent did not opt in by setting MNOTES_SESSION_ID.
if [ -z "\${MNOTES_SESSION_ID:-}" ]; then
  exit 0
fi
