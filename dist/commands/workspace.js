...
async function workspaceList(program) {
...
async function workspaceSelect(program) {
    const globalOpts = program.opts();
    const config = (0, config_1.resolveConfig)(globalOpts);
    const client = (0, client_1.createClient)(config.baseUrl, config.apiKey);
    const res = await client.listWorkspaces();
...
async function workspaceCreate(program, name) {
    const globalOpts = program.opts();
    const config = (0, config_1.resolveConfig)(globalOpts);
    const client = (0, client_1.createClient)(config.baseUrl, config.apiKey);
...
async function workspaceCurrent(program) {
    const stored = (0, login_1.readConfig)();
    if (stored?.workspaceId) {
